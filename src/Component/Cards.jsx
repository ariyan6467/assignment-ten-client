...
const Cards = ({ ai }) => {
    ...
    return (
        <div className="card-surface rounded-3xl overflow-hidden hover:scale-[1.02] w-[240px]  transition-transform duration-300">
            <div className="p-3 ">
                <div className="w-full h-30 rounded-xl overflow-hidden bg-black/30 border border-[var(--border)]">
                    <img src={ai.image} alt={ai.name} className="w-full h-full object-contain" />
                </div>

                <div className="mt-4 space-y-2">
                    <div className="flex items-center justify-between">
                        <h2 className="text-lg font-bold brand-gradient">{ai.name}</h2>
                        <span className="chip px-2 py-1 rounded-full text-xs">{ai.framework}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-3 text-xs text-soft">
                        <span>By: <span className="text-white">{ai.createdBy}</span></span>
